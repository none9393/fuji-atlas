...
def build(source=None,target=None):
    ...
    for symbol,paths in by_symbol.items():
        if paths:
            latest=paths[0].name; featured.append(f'<article><h2>{symbol}</h2><p>En güncel rapor</p><a href="{asset(latest)}">Aç</a><a download href="{asset(latest)}">İndir</a></article>')
    ...
    cache=["./","./index.html",f"./{asset('manifest.webmanifest')}","./health.json",*["./"+asset(p) for p in sorted(pdfs)]]
