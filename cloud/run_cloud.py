...
def position_state(symbol):
    value=os.getenv(f"FUJI_POSITION_{symbol}","UNKNOWN").strip().upper()
    return value if value in {"LONG","SHORT","FLAT"} else "UNKNOWN"
...
def render_pdf(symbol,market,verification,target,report_id,created,evidence,config):
    ...
    cond_text="; ".join(conditions["rsi_flags"]) or "RSI aşırı alım/satım sinyali yok"; conflict="var" if conditions["conflict"] else "yok"; macro="Makro feed bağlı değil; makro olay üretilmedi."
    story=[Paragraph(f"FUJI-ATLAS — {symbol}",title),Paragraph(f"Rapor kimliği: {report_id}<br/>Üretim zamanı: {created.isoformat()}",body),Paragraph("Karar özeti / fırsat ve pozisyon radarı",head),Paragraph("Yönetici özeti · Actionable Intelligence",head),Paragraph("Karar renkleri: YEŞİL · GİR / SARI · TEMKİNLİ / KIRMIZI · GİRME",body),Paragraph(f"Güncel fiyat: {latest.get('close','-')} · Son mum: {'kapalı' if latest.get('is_closed',True) else 'açık'} · Ana yön: {stats.get('direction') if stats else 'hesaplanamadı'}",body),Paragraph("Açık pozisyon desteği",head),Paragraph(f"Repository position variable: {position_state(symbol)}",body),Paragraph("Aktif koşullar ve dikkat noktaları",head),Paragraph(f"Timeframe yön çatışması: {conflict}. {cond_text}. 1H ≥2 ATR mum: {conditions['expansion']}; ≥1.5 ATR boşluk: {conditions['gaps']}. {macro}",body),Paragraph("Piyasa durumu bülteni",head),Paragraph("Bu bültende piyasa durumu; OHLC kapanışları, top-down yön, EMA20/RSI14/ATR14, walk-forward gerçekleşme ve mevcut veri sağlayıcısı/fallback bilgisiyle özetlenir.",body),Paragraph("Fırsat ayrıntıları",head),Paragraph("Top-down karar tablos",head)]
    summary=[["Vade","Karar","Yön","Koşullu giriş","SL","TP1","TP2","Başarı","Tahmini süre"]]
