...
def cross_market_context(market,symbol):
    ...
    if len(common)<20: return {"correlation":None,"sample":len(common),"relation":"yetersiz ortak örneklem"}
    ...
    return {"correlation":corr,"sample":len(common),"relation":"ölçülmüş korelasyon; nedensellik/garanti değildir"}
...
def render_pdf(symbol,market,verification,target,report_id,created,evidence,config):
    ...
    cond_text="; ".join(conditions["rsi_flags"]) or "RSI aşırı alım/satım sinyali yok"; conflict="var" if conditions["conflict"] else "yok"; macro="Makro takvim/haber akışı bağlı değildir; rapor doğrulanmamış haber veya makro neden üretmez."; corr_text=f"{cross['correlation']:.3f} ({cross['sample']} ortak günlük gözlem)" if cross.get('correlation') is not None else f"yayımlanmadı ({cross['sample']} ortak gözlem; en az 20 gerekli)"
    story=[Paragraph(f"FUJI Piyasa Brifingi — {symbol}",title),Paragraph(f"Rapor: {report_id} · {created.isoformat()}",body),Paragraph("1 · Karar ekranı",head),Paragraph("Yönetici özeti · Actionable Intelligence",head),Paragraph("Karar renkleri: YEŞİL · GİR / SARI · TEMKİNLİ / KIRMIZI · GİRME",body),Paragraph(f"Güncel fiyat: {latest.get('close','-')} · Son mum: {'kapalı' if latest.get('is_closed',True) else 'açık'} · Ana yön: {stats.get('direction') if stats else 'hesaplanamadı'}",body),Paragraph("2 · Genel piyasa durumu",head),Paragraph("Piyasa durumu bülteni",head),Paragraph(market_brief(entries,stats),body),Paragraph("Makro ve çapraz piyasa bağlamı",head),Paragraph(f"DXY korelasyonu: {corr_text} — {cross['relation']}. Beklenen yön ilişkisi yalnızca bağlam teyididir. US10Y tek başına sinyal değildir.",body),Paragraph("Açık pozisyon desteği",head),Paragraph("Kullanıcı pozisyon bilgisi verilmedi; rapor yeni pozisyon seviyelerini ve piyasa koşullarını bağımsız değerlendirir.",body),Paragraph("Aktif koşullar ve dikkat noktaları",head),Paragraph(f"Timeframe yön çatışması: {conflict}. {cond_text}. 1H ≥2 ATR mum: {conditions['expansion']}; ≥1.5 ATR boşluk: {conditions['gaps']}. {macro}",body),Paragraph("3 · Fırsat planları",head),Paragraph("Karar tablos",head)]
